...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (file.size > maxSizeBytes) {
      console.error(
        `File size validation failed: ${file.size} bytes exceeds ${maxSizeBytes} bytes for ${userRole} plan`
      );
      throw new Error(
        `File size (${(file.size / 1024 / 1024).toFixed(
          2
        )}MB) exceeds your ${userRole} plan limit of ${
          planConfig.maxFileSizeMB
    ...
    if (dbError) {
      ...
      throw new Error(
        `Failed to create flipbook record: ${
    ...
    const errorMessage =
      error instanceof Error
        ? error.message.includes("R2") || error.message.includes("fetch")
