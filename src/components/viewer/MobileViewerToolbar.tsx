...
// Extend the Window interface to include currentFlipbook
declare global {
  interface Window {
...
const MobileViewerToolbar: React.FC<MobileViewerToolbarProps> = ({
  pdfUrl,
}) => {
  ...
  const callMethod = (
    flipbook: DflipInstance,
    methodPaths: string[],
    args: unknown[] = []
  ) => {
    for (const path of methodPaths) {
      const parts = path.split(".");
      let ctx: unknown = flipbook;
      let fn: unknown = flipbook;
      for (let i = 0; i < parts.length; i++) {
        ctx = fn;
        fn = (fn as Record<string, unknown>)?.[parts[i]];
      }
  ...
  return (
    <div
      className={`rounded-2xl bg-background/95 dark:bg-background/90 shadow-xl flex items-center justify-between px-3 py-2 backdrop-blur-sm transition-all border border-border ${
    ...
    >
      ...
      <Sheet>
        ...
        <SheetContent side="bottom" className="py-6 rounded-t-[20px]">
          <div className="grid grid-cols-1 gap-3">
            ...
            <Button
              onClick={() => doAction("togglePageMode")}
              variant="secondary"
              className="w-full justify-start px-4 h-12"
            >
      ...
      <Button
        ...
        className={`h-9 w-9 transition-colors ${
          zoomLevel > 0 ? "text-blue-600 bg-blue-50" : "text-gray-700"
        }`}
